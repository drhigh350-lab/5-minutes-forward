...
type DialStatus = 'idle' | 'loading' | 'playing' | 'paused' | 'buffering' | 'error';
...
interface PlayDialProps {
  ...
  status?: DialStatus; // when given, overrides the icon for loading/buffering/error feedback
}
  ...
  const dial = (
    <>
      <svg
        ...
        className={`absolute inset-0 -rotate-90 ${isBusy ? 'animate-spin motion-reduce:animate-none' : ''}`}
        style={isBusy ? { animationDuration: '1.1s' } : undefined}
      >
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke={isError ? '#DC2626' : '#C99A3B'}
          strokeWidth={stroke}
          strokeLinecap="round"
          strokeDasharray={isBusy ? `${circumference * 0.28} ${circumference}` : circumference}
          strokeDashoffset={isBusy ? 0 : dashoffset}
          className={isBusy ? '' : 'transition-[stroke-dashoffset] duration-300'}
        />
      </svg>
      <span className="absolute inset-[10px] rounded-full bg-ink flex items-center justify-center transition-colors group-hover:bg-ink-soft">
        {isError ? (
          // Retry glyph — tapping the dial again calls onClick, which
          // callers wire to retry() when status is 'error'.
          <svg width={size * 0.26} height={size * 0.26} viewBox="0 0 24 24" fill="none">
            <path
