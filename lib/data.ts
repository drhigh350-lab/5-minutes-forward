...
/**
 * Lean lookup used only by /api/audio/[slug] on the tap-to-play hot
 * path — selects just the one column that route needs instead of the
 * full episode row, and is small/stable enough to be safely wrapped in
 * unstable_cache there (see that route for why).
 */
export async function getEpisodeAudioKeyBySlug(slug: string): Promise<string | null> {
  const supabase = createPublicClient();
  const { data, error } = await supabase
    .from('episode')
  ...
  if (error) throw error;
  return (data?.audio_object_key as string | undefined) ?? null;
}
